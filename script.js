document.addEventListener("DOMContentLoaded", () => {
  ...
  function formatKyivTime(date){
    ...
    return str.replace(/(\d{2})\.(\d{2})\.(\d{2}),? (\d{2}):(\d{2})/, "$4:$5");
  }
  ...
  function updateReport(){
   ...
   if(resultRadio.value === "destroyed") {
    ...
    const reportText=
`${time} МГП "Смерека" зрадн 15 броп НГУ, МОНАЧИНІВКА Виявлено ${type} ( А-${azimuth}°, К-${course}°, В-${height}m, Д-${distance}m ). Ціль обстріляно з АК-74. Ціль ${result}. Витрата набоїв: 
